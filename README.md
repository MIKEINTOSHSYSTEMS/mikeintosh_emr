# MHC - EMR by Installing Odoo 16.0 or Odoo 17.0 with one command (Supports multiple Odoo instances on one server).

## Quick Installation

Install [docker](https://docs.docker.com/get-docker/) and [docker-compose](https://docs.docker.com/compose/install/) yourself, then run the following to set up first Odoo instance @ `localhost:10017` (default master password: `momonahealthcare`):

``` bash
curl -s https://raw.githubusercontent.com/MIKEINTOSHSYSTEMS/mikeintosh_emr/main/run.sh | sudo bash -s odoo-one 10017 20017
```
and/or run the following to set up another Odoo instance @ `localhost:11017` (default master password: `momonahealthcare`):

``` bash
curl -s https://raw.githubusercontent.com/MIKEINTOSHSYSTEMS/mikeintosh_emr/main/run.sh | sudo bash -s odoo-two 11017 21017
```
...
* Default database password (**admin_passwd**) is `momonahealthcare`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
